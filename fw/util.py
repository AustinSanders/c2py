...
def get_local_class(class_name):
    ...
    mod_description, c_name = class_name.split('.')
    mod = __import__(mod_description)
...
def connect(interface_pair, dispatcher, id):
    ...
    if isinstance(dispatcher, list):
        el = fw.EventListener(id, dispatcher[0].get_event_dispatcher(dispatcher[1]))
    elif isinstance(dispatcher, fw.EventListener):
        el = dispatcher
    else:
